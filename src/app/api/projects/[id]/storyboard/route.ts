...
import { reviseStoryboard, improveStoryboard, resolveBrainAuth } from "@/lib/brain";
...
/**
 * Rewrite the whole storyboard — either from a director's note (`feedback`), or via
 * `auto: true` (Claude self-critiques against the playbook and improves it). Resets
 * stills/clips (this is the cheap, pre-generation iteration step).
 */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { feedback, auto } = await req.json().catch(() => ({}));
  if (!auto && !String(feedback || "").trim()) return NextResponse.json({ error: "Add a note, or use Improve with AI." }, { status: 400 });
  ...
  try {
    sb = auto
      ? await improveStoryboard(common, resolveBrainAuth(user))
      : await reviseStoryboard({ ...common, feedback: String(feedback) }, resolveBrainAuth(user));
  } catch (e) {
